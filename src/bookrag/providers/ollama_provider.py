...
class OllamaProvider:
    def __init__(
        ...
        # better-informed decision with a worse one. Set $OLLAMA_NUM_GPU only
        # to force the issue - typically when Ollama's headroom estimate is
        # conservative and a full offload nearly fits. Asking for more layers
        # than genuinely fit fails loudly (an out-of-memory error from the
        # server), it does not quietly fall back.
        self._num_gpu = num_gpu if num_gpu is not None else env_optional_int("OLLAMA_NUM_GPU")
